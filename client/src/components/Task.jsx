        ...
        <div className={`status-${ props.task.status.toLowerCase() } my-3 py-2 px-3 border border-secondary rounded-md`}>
            ...
            <div className='flex flex-row justify-between items-center'>
                ...
                <div className='flex flex-col items-end text-xs text-neutral-500'>
                    <p>Create on { DateTime.fromJSDate(new Date(props.task.createdAt)).toLocaleString(DateTime.DATETIME_MED_WITH_SECONDS) }</p>
                    <p>Updated on { DateTime.fromJSDate(new Date(props.task.updatedAt)).toLocaleString(DateTime.DATETIME_MED_WITH_SECONDS) }</p>
                </div>
            </div>
